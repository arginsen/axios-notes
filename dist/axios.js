	...
	function forEach(obj, fn) {
	  ...
	  if (isArray(obj)) {
	    ...
	    for (var i = 0, l = obj.length; i < l; i++) {
	      fn.call(null, obj[i], i, obj); // 函数fn的this指向null, 再将其后的参数（值、序号、数组）传入fn
	    }
	  } else {
	    // Iterate over object keys
	    for (var key in obj) {
	      if (Object.prototype.hasOwnProperty.call(obj, key)) { // 过滤可遍历的属性
	...
	function extend(a, b, thisArg) {
	  forEach(b, function assignValue(val, key) {
	    if (thisArg && typeof val === 'function') {
	      a[key] = bind(val, thisArg); // 如果值为函数，使函数this指向a对象
	    } else {
	      a[key] = val;
	    }
	  });
...
/***/ (function(module, exports, __webpack_require__) {
	...
	InterceptorManager.prototype.forEach = function forEach(fn) {
	  utils.forEach(this.handlers, function forEachHandler(h) { // h为每个拦截器函数作为参数
	    if (h !== null) {
	      fn(h);
	    }
	  });
...
/***/ (function(module, exports) {
	 ...
	 *  // spread作为函数被导出，参数传入回调函数，又该函数返回wrap函数，传入参数立即执行
	 *  // 返回回调函数绑定了之前的外界传入参数
	 *  ```
	 *
